...
app.post('/api/challenges/:id/respond',auth,async(req,res)=>{ const c=await q(`SELECT c.*,u.username AS challenger_name FROM challenges c JOIN users u ON u.id=c.challenger WHERE c.id=$1 AND c.opponent=$2 AND c.status='pending' AND c.expires_at>now()`,[req.params.id,req.user.id]); if(!c.rows[0])return res.status(404).json({error:'Izazov nije pronađen ili je istekao.'}); if(req.body.status!=='accepted'){await q(`UPDATE challenges SET status='declined' WHERE id=$1`,[req.params.id]);return res.json({ok:true});} await q(`UPDATE challenges SET status='accepted' WHERE id=$1`,[req.params.id]); const id=newGame(c.rows[0].challenger,c.rows[0].opponent,{playerNames:{w:c.rows[0].challenger_name,b:req.user.username}}); io.to(`user:${c.rows[0].challenger}`).emit('challenge-accepted',{gameId:id}); res.json({ok:true,gameId:id}); });

function newGame(a=null,b=null,options={}){ const id=crypto.randomUUID(); games.set(id,{id,chess:new Chess(),players:{w:a,b},playerNames:{w:options.playerNames?.w||'Anonimni igrač',b:options.playerNames?.b||null},progress:{w:0,b:0},anonProgress:{},drawOffer:null,unlocked:false,lastMove:null,bot:options.bot||null,botThinking:false,createdAt:Date.now()}); return id; }
