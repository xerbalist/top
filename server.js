...
const optionalAuth = async (req,res,next)=>{
  ...
  if (!session) {
    if (requestToken(req)) {
      ...
      return res.status(401).json({error:'Sesija je istekla ili je nalog onemogućen. Prijavi se ponovo.'});
    }
...
installAdmin(app, {auth,q,sessions,limiter,stopGames:userId=>{
  for (const [id,g] of games) {
    if (!isMember(g,userId)) continue;
    if (g.botTimer) clearTimeout(g.botTimer);
    g.ended=true;g.unlocked=false;delete g.chatKeys;
    io.to(id).emit('ended',{reason:'partija je zatvorena'});
...
app.post('/api/friends/request',auth,async(req,res)=>{ try { if(!limiter.take('friend-request:'+req.user.id,5,60000))return res.status(429).json({error:'Sačekaj pre slanja novog zahteva.'}); if(!req.body.userId||req.body.userId===req.user.id)return res.status(400).json({error:'Neispravan korisnik.'}); const existing=await q(`SELECT status FROM friendships WHERE (requester=$1 AND addressee=$2) OR (requester=$2 AND addressee=$1) ORDER BY (status='blocked') DESC LIMIT 1`,[req.user.id,req.body.userId]); if(existing.rows[0]?.status==='blocked')return res.status(403).json({error:'Zahtev nije moguć zbog blokade.'}); if(existing.rows[0]?.status==='accepted')return res.status(409).json({error:'Već ste prijatelji.'}); if(existing.rows[0]?.status==='pending')return res.status(409).json({error:'Zahtev već postoji.'}); await q(`DELETE FROM friendships WHERE ((requester=$1 AND addressee=$2) OR (requester=$2 AND addressee=$1)) AND status<>'blocked'`,[req.user.id,req.body.userId]); const inserted=await q(`INSERT INTO friendships(requester,addressee,status) SELECT $1,$2,'pending' WHERE EXISTS(SELECT 1 FROM users WHERE id=$2 AND NOT blocked) AND NOT EXISTS(SELECT 1 FROM friendships WHERE ((requester=$1 AND addressee=$2) OR (requester=$2 AND addressee=$1)) AND status='blocked') ON CONFLICT DO NOTHING RETURNING id`,[req.user.id,req.body.userId]); if(!inserted.rows[0])return res.status(409).json({error:'Zahtev nije moguće poslati.'}); io.to('user:'+req.body.userId).emit('friend-request-received',{id:String(inserted.rows[0].id),requester:req.user.id,username:req.user.username}); res.json({ok:true}); } catch { res.status(400).json({error:'Zahtev nije moguće poslati.'}); }});
...
app.get('/api/friends/requests',auth,async(req,res)=>{
  const r=await q(`SELECT f.id,f.requester,u.username FROM friendships f JOIN users u ON u.id=f.requester
    WHERE f.addressee=$1 AND f.status='pending' AND NOT u.blocked
    AND NOT EXISTS(SELECT 1 FROM friendships b WHERE ((b.requester=f.requester AND b.addressee=f.addressee) OR (b.requester=f.addressee AND b.addressee=f.requester)) AND b.status='blocked')
    ORDER BY f.created_at ASC LIMIT 50`,[req.user.id]);
  res.json(r.rows);
});
app.post('/api/friends/respond',auth,async(req,res)=>{
  if(!['accepted','declined'].includes(req.body.status))return res.status(400).json({error:'Neispravan odgovor.'});
  const r=await q(`UPDATE friendships f SET status=$1 WHERE requester=$2 AND addressee=$3 AND status='pending'
    AND ($4::text IS NULL OR f.id::text=$4)
    AND EXISTS(SELECT 1 FROM users u WHERE u.id=f.requester AND NOT u.blocked)
    AND NOT EXISTS(SELECT 1 FROM friendships b WHERE ((b.requester=f.requester AND b.addressee=f.addressee) OR (b.requester=f.addressee AND b.addressee=f.requester)) AND b.status='blocked')
    RETURNING id`,[req.body.status,req.body.id,req.user.id,req.body.requestId==null?null:String(req.body.requestId)]);
  if(!r.rows[0])return res.status(404).json({error:'Zahtev više nije dostupan.'});
  io.to('user:'+req.user.id).emit('friend-request-resolved',{id:String(r.rows[0].id)});
  res.json({ok:true});
});
...
app.post('/api/challenges',auth,async(req,res)=>{ if(!limiter.take('challenge-send:'+req.user.id,5,60000))return res.status(429).json({error:'Sačekaj pre slanja novog izazova.'}); const relation=await q(`SELECT 1 FROM friendships WHERE ((requester=$1 AND addressee=$2) OR (requester=$2 AND addressee=$1)) AND status='accepted'`,[req.user.id,req.body.userId]); if(!relation.rows[0])return res.status(403).json({error:'Izazov možeš poslati samo prijatelju.'}); const blocked=await q(`SELECT 1 FROM friendships WHERE ((requester=$1 AND addressee=$2) OR (requester=$2 AND addressee=$1)) AND status='blocked'`,[req.user.id,req.body.userId]); if(blocked.rows[0])return res.status(403).json({error:'Izazov nije moguć zbog blokade.'}); const id=crypto.randomUUID(); const inserted=await q(`INSERT INTO challenges(id,challenger,opponent,expires_at) VALUES($1,$2,$3,now()+interval '24 hours') RETURNING expires_at`,[id,req.user.id,req.body.userId]); io.to('user:'+req.body.userId).emit('challenge-received',{id,challenger:req.user.username,expires_at:inserted.rows[0].expires_at}); res.json({id}); });
app.get('/api/challenges',auth,async(req,res)=>{ const r=await q(`SELECT c.id,c.status,c.created_at,c.expires_at,u.username AS challenger FROM challenges c JOIN users u ON u.id=c.challenger WHERE c.opponent=$1 AND c.status='pending' AND c.expires_at>now() AND NOT u.blocked AND EXISTS(SELECT 1 FROM friendships f WHERE ((f.requester=c.challenger AND f.addressee=c.opponent) OR (f.requester=c.opponent AND f.addressee=c.challenger)) AND f.status='accepted') AND NOT EXISTS(SELECT 1 FROM friendships f WHERE ((f.requester=c.challenger AND f.addressee=c.opponent) OR (f.requester=c.opponent AND f.addressee=c.challenger)) AND f.status='blocked') ORDER BY c.created_at DESC LIMIT 50`,[req.user.id]); res.json(r.rows); });
app.post('/api/challenges/:id/respond',auth,async(req,res)=>{
  if(req.body.status!=='accepted'){await q("UPDATE challenges SET status='declined' WHERE id=$1 AND opponent=$2 AND status='pending'",[req.params.id,req.user.id]);io.to('user:'+req.user.id).emit('challenge-resolved',{id:req.params.id});return res.json({ok:true});}
  const c=await q(`UPDATE challenges c SET status='accepted' FROM users u WHERE c.id=$1 AND c.opponent=$2 AND c.status='pending' AND c.expires_at>now() AND u.id=c.challenger AND NOT u.blocked AND EXISTS(SELECT 1 FROM friendships f WHERE ((f.requester=c.challenger AND f.addressee=c.opponent) OR (f.requester=c.opponent AND f.addressee=c.challenger)) AND f.status='accepted') AND NOT EXISTS(SELECT 1 FROM friendships f WHERE ((f.requester=c.challenger AND f.addressee=c.opponent) OR (f.requester=c.opponent AND f.addressee=c.challenger)) AND f.status='blocked') RETURNING c.*,u.username AS challenger_name`,[req.params.id,req.user.id]);
  ...
  io.to('user:'+req.user.id).emit('challenge-resolved',{id:req.params.id});
