...
import { advanceSequence, validatePersonalSequence } from './lib/move-sequence.js';
...
app.post('/api/profile/secret-moves/validate',auth,async(req,res)=>{ try { const moves=Array.isArray(req.body.moves)?req.body.moves:[]; const result=validatePersonalSequence(moves); res.json({valid:true,complete:moves.length===5,fen:result.fen,history:result.history}); } catch { res.status(400).json({valid:false,error:'Sekvenca sadrži nelegalan potez.'}); }});
app.put('/api/profile/secret-moves',auth,async(req,res)=>{ try { const moves=Array.isArray(req.body.moves)?req.body.moves:[]; if(moves.length!==5) return res.status(400).json({error:'Moraš uneti tačno pet legalnih poteza.'}); const result=validatePersonalSequence(moves); await q('UPDATE users SET secret_moves=$1 WHERE id=$2',[JSON.stringify(result.history),req.user.id]); res.json({ok:true,message:'Pet poteza je sačuvano.'}); } catch { res.status(400).json({error:'Pet poteza nije sačuvano.'}); }});
...
io.on('connection',socket=>{ socket.data.playerId=socketIdentity(socket); socket.on('join-game',id=>{ const g=findGame(id); if(!g||![g.players.w,g.players.b].includes(socket.data.playerId))return; socket.join(g.id); socket.data.game=g.id; }); socket.on('chat-message',({gameId,text}={})=>{ const g=findGame(gameId); const clean=String(text||'').trim(); if(!g?.unlocked||g.ended||g.chess.isGameOver()||![g.players.w,g.players.b].includes(socket.data.playerId)||!clean||clean.length>1000)return; io.to(g.id).emit('chat-message',{text:clean,at:Date.now()}); }); });
