...
export function createSessions({ secret, query, disconnect = () => {} }) {
  ...
  return {
    async issue(user) {
      ...
      const r=await query("INSERT INTO sessions(id,user_id,expires_at,auth_version) SELECT $1,id,now()+interval '30 days',auth_version FROM users WHERE id=$2 AND auth_version=$3 AND NOT blocked RETURNING id", [sid,user.id,user.authVersion||0]);
    ...
    async read(token) {
      ...
      const r = await query('SELECT u.id,u.username FROM sessions s JOIN users u ON u.id=s.user_id WHERE s.id=$1 AND s.user_id=$2 AND s.expires_at>now() AND s.auth_version=u.auth_version AND NOT u.blocked', [claims.sid,claims.id]);
