...
export function installSocial(app,{auth,q,canSeeStatus,limiter}) {
  app.post('/api/link-preview',auth,async(req,res)=>{
    if(!limiter.take('preview:'+req.user.id,30,60000))return res.status(429).json({error:'Sačekaj pre učitavanja novog pregleda.'});
    res.json(await linkPreview(String(req.body.url||'')));
  });
  app.post('/api/statuses',auth,async(req,res)=>{
    ...
    const preview=media.link?await linkPreview(media.link):{};
    const result=await q('INSERT INTO statuses(user_id,body,image_data,link_url,link_preview) VALUES($1,$2,$3,$4,$5) RETURNING id,body,created_at',[req.user.id,body,media.image,media.link,JSON.stringify(preview)]);
    res.status(201).json(result.rows[0]);
  });
  app.get('/api/statuses/:id/link-image',auth,canSeeStatus,async(req,res)=>{
    const r=await q('SELECT link_preview FROM statuses WHERE id=$1',[req.params.id]);
    const m=r.rows[0]?.link_preview?.image?.match(/^data:(image\/(?:png|jpeg|webp));base64,(.+)$/);
  ...
  app.get('/api/feed',auth,async(req,res)=>{
    ...
    const result=await q(`SELECT s.id,s.body,s.created_at,s.link_url,(s.link_preview - 'image') AS link_preview,(COALESCE(s.link_preview->>'image','')<>'') AS has_link_image,(s.image_data<>'') AS has_image,u.username,
